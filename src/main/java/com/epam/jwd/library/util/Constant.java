package com.epam.jwd.library.util;

public class Constant {

    public static final String ID_PARAMETER_NAME = "id";
    public static final String LOGIN_PARAMETER_NAME = "login";
    public static final String PASSWORD_PARAMETER_NAME = "password";
    public static final String ROLE_PARAMETER_NAME = "role";
    public static final String BOOK_PARAMETER_NAME = "book";
    public static final String BOOK_ORDERS_ATTRIBUTE_NAME = "bookOrders";
    public static final String FIRST_NAME_PARAMETER_NAME = "first_name";
    public static final String LAST_NAME_PARAMETER_NAME = "last_name";
    public static final String TITLE_PARAMETER_NAME = "title";
    public static final String DATE_PUBLISHED_PARAMETER_NAME = "date_published";
    public static final String AMOUNT_OF_LEFT_PARAMETER_NAME = "amount_of_left";
    public static final String ACCOUNT_PARAMETER_NAME = "account";
    public static final String ACCOUNTS_ATTRIBUTE_NAME = "accounts";
    public static final String BOOKS_ATTRIBUTE_NAME = "books";
    public static final String AUTHOR_ATTRIBUTE_NAME = "author";
    public static final String BOOK_ID_PARAMETER_NAME = "book_id";
    public static final String ORDER_TYPE_PARAMETER_NAME = "order_type";
    public static final String AUTHORS_ATTRIBUTE_NAME = "authors";
    public static final String BOOK_ORDER_ATTRIBUTE_NAME = "bookOrders";
    public static final String ERROR_PASS_MESSAGE_ATTRIBUTE_NAME = "errorPassMessage";
    public static final String ERROR_LOGIN_PASS_MESSAGE_NAME = "errorLoginPassMessage";
    public static final String ERROR_REGISTER_PASS_MESSAGE_NAME = "errorRegistrationMessage";
}